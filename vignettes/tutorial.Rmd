...
```{r setup Python, message=FALSE, warning=FALSE, results='hide'}
...
```{r pseudo outcome, message=FALSE, warning=FALSE}
...
DR_WCLS_R_df = UI_return_R %>%
  transmute(
    term  = E,
    method = "DR_WCLS_LASSO (R)",
    est   = GEE_est,
    low   = lowCI,
    up    = upperCI
  )
# DR_WCLS_Py_df = UI_return_python %>%
#   transmute(
#     term  = E,
#     method = "DR_WCLS_LASSO (Python)",
#     est   = GEE_est,
#     low   = lowCI,
#     up    = upperCI
#   )

wcls_df = as.data.frame(wcls_res$causal_excursion_effect) %>%
...
box_plot_df = bind_rows(DR_WCLS_R_df, 
                        # DR_WCLS_Py_df, 
                        wcls_df)

intercept_df = box_plot_df[box_plot_df$term == '(Intercept)',]
ggplot(intercept_df,
       aes(x = term, y = est, color = method,
           group = method)) +
...
day_in_study_df = box_plot_df[box_plot_df$term == 'day_in_study',]
ggplot(day_in_study_df,
       aes(x = term, y = est, color = method,
           group = method)) +
  geom_pointrange(aes(ymin = low, ymax = up),
                  position = position_dodge(width = 0.5),
...
**Methods of generating the pseudo outcome**
The pseudo outcome generation method can be specified using the `method_pseu` argument. Currently, three options are supported: CVLASSO, RandomForest, and GradientBoosting. The default is CVLASSO.
...
**Availability**
We also allow users to specify an availability indicator through the `availability` argument. Users can set this argument to the name of the availability column in the dataset. When provided, treatment is only considered at decision points when a participant is available.
...
wcls_sim = as.data.frame(wcls_sim_res$causal_excursion_effect[1:5,]) %>%
  rownames_to_column("term") %>%       
  transmute(
    term,
    method = "WCLS",
...
box_plot_sim = bind_rows(DR_WCLS_R_sim, 
                        # DR_WCLS_Py_df, 
                        wcls_sim)

intercept_df = box_plot_sim[box_plot_sim$term == '(Intercept)',]
ggplot(intercept_df,
       aes(x = term, y = est, color = method,
           group = method)) +
  geom_pointrange(aes(ymin = low, ymax = up),
...
state1_df = box_plot_sim[box_plot_sim$term == 'state1',]
ggplot(state1_df,
       aes(x = term, y = est, color = method,
           group = method)) +
  geom_pointrange(aes(ymin = low, ymax = up),
...
state2_df = box_plot_sim[box_plot_sim$term == 'state2',]
ggplot(state2_df,
       aes(x = term, y = est, color = method,
           group = method)) +
  geom_pointrange(aes(ymin = low, ymax = up),
...
state3_df = box_plot_sim[box_plot_sim$term == 'state3',]
ggplot(state3_df,
       aes(x = term, y = est, color = method,
           group = method)) +
  geom_pointrange(aes(ymin = low, ymax = up),
...
state4_df = box_plot_sim[box_plot_sim$term == 'state4',]
ggplot(state4_df,
       aes(x = term, y = est, color = method,
           group = method)) +
  geom_pointrange(aes(ymin = low, ymax = up),
