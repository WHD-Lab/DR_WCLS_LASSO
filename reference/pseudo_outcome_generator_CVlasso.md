...
function uses CV LASSO to conduct model training on folds then estimates
\\E\[Y\_{t+1}\|H_t, A_t\], E\[A_t\|H_t\], E\[A_t\|S_t\]\\ for the
reserved fold. Then the function returns a dataset with column named
...
appropriate working models with CV LASSO and generates pseudo outcome
using the DR-WCLS.
...
pseudo_outcome_generator_CVlasso(fold = 5,ID = "id", sim_data, Ht, St, "action", "outcome",core_num = 5)
