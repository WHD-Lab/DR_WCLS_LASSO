...
function uses Gradient Boosting to conduct model training on folds then
...
appropriate working models with Gradient Boosting and generates pseudo
...
#> Error in pseudo_outcome_generator_gbm(fold = 5, ID = "id", sim_data, Ht,     St, "action", "outcome", core_num = 5): argument "outcome" is missing, with no default
```
